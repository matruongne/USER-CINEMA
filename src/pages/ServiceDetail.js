...
const ServiceDetail = () => {
	...
	return (
		<div className="service-detail">
			<img src={detail.image} alt={detail.title} className="service-detail-img w-full" />
			<h2 className="service-detail-title">{detail.title}</h2>
			<p className="service-detail-description">{detail.description}</p>
			<button
				className="contact-btn"
				onClick={() => (window.location.href = 'mailto:Beoruiden1231242125@gmail.com')}
			>
				LIÊN HỆ NGAY
			</button>
		</div>
