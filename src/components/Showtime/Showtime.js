    ...
    if (selectedMovie) {
      ...
      if (selectedDate) {
        filtered = filtered.filter(
          (movie) => movie.showtimes[selectedDate]?.length > 0
        );
      }
      ...
      if (selectedCinema) {
        filtered = filtered.filter((movie) =>
          movie.cinemas.includes(selectedCinema.id)
        );
      }
    }
  ...
  const handleShowtimeClick = (movie, date, time) => {
    try {
      ...
      const bookingInfo = {
        ...
        cinemaName:
          selectedCinema?.name ||
          cinemas.find((c) => c.id === movie.cinemas[0])?.name,
        showDate: date,
        showTime: time,
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="space-y-8">
        {filteredMovies.map((movie) => (
          <div
            key={movie.id}
            className="bg-bgColor rounded-lg shadow-lg overflow-hidden"
          >
